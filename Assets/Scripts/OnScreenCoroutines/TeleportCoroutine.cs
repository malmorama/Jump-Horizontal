...
//activiated via an event on the player logic script
public class TeleportCoroutine : MonoBehaviour
{
    ...
    private IEnumerator TelePortPlatformCoroutine()
    {
        ...
        //jumpForce = 0;
        rb2d.gravityScale = 0;
        player.transform.position = new Vector2(gameObject.transform.position.x, -45);
        ...
        player.transform.position = tempPlat.transform.position;
        //yield return new WaitForSeconds(0.5f);
        rb2d.gravityScale = 5;
        //jumpForce = 800;
        rb2d.AddForce(Vector2.up * gameVariables.jumpForce);
        gameVariables.jumpForce = 0f;
